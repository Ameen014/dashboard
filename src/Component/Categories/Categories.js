...
const Categories = () => {
    ...
    const BaseApi = api_Routes.category.view
                ...
                setdress(prev=>[...prev,{
                    ...
                    action: (
                        <div>
                          ...
                          <span style={{paddingLeft:"15px"}} onClick={()=>{navigate(`/Editcategory/${elem.id}`)}}>
    ...
    const handleDeleteConfirmed = async () => {
        const {response, message} = await Helper.Delete({
          url:api_Routes.Categories.bulkDelete(recordIdToDelete),
          hasToken:true,
        })
        if(response){
    ...
    return(
        ...
        <Container sx={{marginBottom:"20px"}}>
            <Grid container sx={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <Grid item>
                  <Typography sx={{fontSize:"28px" , fontWeight:"600" , color:"#1e1b1b"}} >Categories</Typography>
                </Grid>
                <Grid item >
                  <Button variant="contained" startIcon={<AddIcon />} sx={{backgroundColor:"#14213D ",fontSize:"13px",borderRadius:"7px",height:"38px",'&:hover': {  backgroundColor: "#14213D "  }}} onClick={()=> {navigate('/Addcategory')}}>
                    Add category
                  </Button>
                </Grid>
            </Grid>
