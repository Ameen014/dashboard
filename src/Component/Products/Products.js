...
const Products = () => {
    ...
    const BaseApi = api_Routes.products.view
    ...
    const get_product = async (signal) => {
            ...
            setTotalItems(response.meta.total)
              ...
              response.data.forEach(elem => {
                setproduct(prev=>[...prev,{
                    ...
                    visible:elem.visible === 1 ? "True" : "False",
                    action: (
                        <div>
                          ...
                          <span style={{paddingLeft:"15px"}} onClick={()=>{navigate(`/EditProduct/${elem.id}`)}}>
    ...
    const handleDeleteConfirmed = async () => {
        const {response, message} = await Helper.Delete({
          url:api_Routes.productes.bulkDelete(recordIdToDelete),
          hasToken:true,
        })
        if(response){
    ...
    return(
        ...
        <Container sx={{marginBottom:"20px"}}>
            <Grid container sx={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <Grid item>
                  <Typography sx={{fontSize:"28px" , fontWeight:"600" , color:"#1e1b1b"}} >Products</Typography>
                </Grid>
                <Grid item >
                  <Button variant="contained" startIcon={<AddIcon />} sx={{backgroundColor:"#14213D ",fontSize:"13px",borderRadius:"7px",height:"38px",'&:hover': {  backgroundColor: "#14213D "  }}} onClick={()=> {navigate('/AddProduct')}}>
                    Add Product
                  </Button>
                </Grid>
            </Grid>
                                    ...
                                    <TableBody>
                                        {product.map((ad) => (
                                            <TableRow key={ad.id}>
                                                ...
                                                <TableCell>{ad.visible}</TableCell>
                                                <TableCell>{ad.category_name}</TableCell>
