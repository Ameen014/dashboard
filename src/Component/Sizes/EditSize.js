...
const EditSize = () => {
    ...
    const get_sizes = async ()=>{
        ...
        const {response,message} = await Helper.Get({
            url:api_Routes.sizes.getOne(sizeid),
            hasToken:true,
        })
        if(response){
    ...
    return;
        ...
        const { response, message } = await Helper.Post({
            url: api_Routes.sizes.update(sizeid),
            data: form_data,
            hasToken: true
        });
        ...
        return(
            <>
                ...
                <Grid container sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                    <Grid item>
                        <Typography sx={{ fontSize: "28px", fontWeight: "600", color: "#1e1b1b" }}>Edit Specification</Typography>
                    </Grid>
                    <Grid item>
                    <Button 
                        variant="contained" 
                    ...
                    <h3 style={{fontWeight:500,marginBottom:"30px"}}>Basic Information</h3>
                        <Box component="form" noValidate autoComplete="off">
                            ...
                            <Grid item xs={12} sm={4}>
                                  <InputLabel className="inputlable">Size Name</InputLabel>
                                    <TextField 
                                        label=" name" 
                                          size="small"
                                        variant="outlined" 
                                ...
                                <Grid item xs={12} sm={4}>
                                  <InputLabel className="inputlable">Size Price</InputLabel>
                                    <TextField 
                                        label="price" 
                                        size="small"
                                        variant="outlined" 
