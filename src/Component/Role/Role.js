...
const Role = () => {
    ...
    const BaseApi = api_Routes.role.view
            ...
            setTotalItems(response.meta.total)
              ...
              response.data.forEach(elem => {
                  setrole(prev=>[...prev,{
                    id:elem.id,
                    name:elem.name,
                    website:elem.website === 1 ? "Ture" : "False",
                    ...
                    action: (
                        <div>
                          ...
                          <span style={{paddingLeft:"15px"}} onClick={()=>{navigate(`/EditRole/${elem.id}`)}}>
                          ...
                          <span style={{paddingLeft:"14px",paddingTop:"10px"}} onClick={()=>{navigate(`/Permission/${elem.name}/${elem.id}`)}} className="permission-icon">
                            <svg width="26" height="22" viewBox="0 0 24 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M23.0435 6.04734C15.3093 5.25164 13.0814 0.795703 12.9859 0.604734C12.7949 0.222797 12.4448 0 12.031 0C11.6173 0 11.2353 0.254625 11.0762 0.604734C10.9489 0.795703 8.68907 5.25164 0.954843 6.04734C0.413765 6.111 0 6.55659 0 7.1295V14.9274C0 24.0939 11.0444 29.8866 11.5218 30.1412C11.6809 30.2049 11.8401 30.2685 11.9992 30.2685C12.1583 30.2685 12.3493 30.2367 12.4766 30.1412C12.954 29.8866 23.9984 24.1257 23.9984 14.9274V7.09767C24.0302 6.55659 23.5846 6.111 23.0435 6.04734ZM21.8659 14.9274C21.8659 21.7704 14.068 26.7356 11.9992 27.9451C9.93037 26.7356 2.13248 21.8023 2.13248 14.9274V8.05251C7.7024 7.25681 10.6306 4.6469 11.9992 2.99184C13.3678 4.6469 16.296 7.25681 21.8659 8.05251V14.9274Z" fill="gray"/>
                            <path d="M7.86133 11.5854C7.86133 13.4951 9.16628 15.0865 10.9168 15.5639V23.2345C10.9168 23.8392 11.3942 24.3167 11.999 24.3167C12.6037 24.3167 13.0811 23.8392 13.0811 23.2345V22.025H14.4179C15.0227 22.025 15.5001 21.5476 15.5001 20.9429C15.5001 20.3381 15.0227 19.8607 14.4179 19.8607H13.0811V15.5639C14.8317 15.0865 16.1366 13.4951 16.1366 11.5854C16.1366 9.29379 14.2906 7.44775 11.999 7.44775C9.70736 7.47958 7.86133 9.32561 7.86133 11.5854ZM13.9723 11.5854C13.9723 12.6676 13.0811 13.5588 11.999 13.5588C10.9168 13.5588 10.0256 12.6676 10.0256 11.5854C10.0256 10.5033 10.9168 9.61207 11.999 9.61207C13.0811 9.61207 13.9723 10.5033 13.9723 11.5854Z" fill="gray"/>
                            </svg>
                          </span>
    ...
    return(
        ...
        <Container sx={{marginBottom:"20px"}}>
            <Grid container sx={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <Grid item>
                  <Typography sx={{fontSize:"28px" , fontWeight:"600" , color:"#1e1b1b"}} >Role</Typography>
                </Grid>
                <Grid item >
                  <Button variant="contained" startIcon={<AddIcon />} sx={{backgroundColor:"#14213D ",fontSize:"13px",borderRadius:"7px",height:"38px",'&:hover': {  backgroundColor: "#14213D "  }}} onClick={()=> {navigate('/AddRole')}}>
                    Add Role
                  </Button>
                </Grid>
            </Grid>
             ...
             <Box component="form" noValidate autoComplete="off">
                <Grid container spacing={2}>
                  ...
                  <Grid item  xs={12} sm={1} sx={{marginTop:"5px"}} >
                      <Button variant="contained" sx={{backgroundColor:"#14213D ",textAlign:"right" ,fontSize:"13px",borderRadius:"7px",height:"38px",'&:hover': {  backgroundColor: "#14213D "  }}} onClick={()=> {handleFilter()}}>
                         <ManageSearchIcon />
                      </Button>
                  </Grid>
              </Grid>
        ...
        {isLoading ? <LinearProgress  sx={{marginTop:"30px",color:"#14213D "}} /> :  
                                    ...
                                    <TableBody>
                                        {role.map((role) => (
                                            <TableRow key={role.id}>
                                                ...
                                                <TableCell>{role.website}</TableCell>
                                                <TableCell>{new Date(role.created_at).toLocaleDateString()  }</TableCell>
                                                <TableCell>{role.action  }</TableCell>               
                                            </TableRow>
                                        ))}
                                    </TableBody>
                                </Table>
